...
function pakaiHint(){
  ...
  // Isi otomatis huruf-huruf yang memang sudah "dibocorkan" (revealed) dari
  // posisi sekarang, lalu berhenti TEPAT setelah membuka 1 huruf yang
  // sebelumnya benar-benar tersembunyi. Ini memastikan 1 kesempatan hint
  // selalu memberi info baru -- tidak terbuang percuma kalau posisi
  // berikutnya kebetulan sama dengan huruf bocoran yang sudah terlihat.
  let i = state.typed.length;
  let newTyped = state.typed;
  while(i < state.answer.length){
    const wasHidden = !state.revealed.has(i);
    newTyped += state.answer[i];
    i++;
    if(wasHidden) break;
  }
...
function updateHintBtn(){
  ...
  btn.textContent = state.hintCharges>0
    ? `💡 Petunjuk (${state.hintCharges} kesempatan)`
    : '💡 Petunjuk (habis)';
}
